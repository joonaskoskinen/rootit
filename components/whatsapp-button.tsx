...
export function WhatsAppButton() {
  ...
  useEffect(() => {
    // Show button after a short delay for nice entrance
    const timer = setTimeout(() => setIsVisible(true), 1000)
    return () => clearTimeout(timer)
  }, [])

  const phoneNumber = "358405256014"
  const message = "Hei! Olisin kiinnostunut IT-palveluistanne."
  ...
  return (
    <a
      ...
      className={`
        ...
        ${isHovered ? 'sm:w-auto sm:h-auto sm:px-5 sm:py-3 sm:gap-3' : ''}
      `}
    ...
    >
      ...
      {/* Expandable text - only on desktop */}
      <span
        className={`
